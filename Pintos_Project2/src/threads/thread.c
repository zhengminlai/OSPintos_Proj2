               ...
               thread_func *function, void *aux) 
{
...
#ifdef USERPROG
  if (list_size (&all_list) >= 35) /* if the list size>=35,return Create_Error */
    return TID_ERROR;
#endif
 ...
 //Added by Lai ZhengMin and JinXin to initialize the variables declared in struct thread
#ifdef USERPROG
  sema_init (&t->wait, 0);
...
//Added by Jiang LinXi to calculate the thread's priority
void
...
{   
    ...
    sr.t = cur;
    sr.tick = ticks + timer_ticks();
	old_level = intr_disable(); 
    list_insert_ordered(&sleep_list, &sr.elem, tick_less, NULL);
    if (ticks < max_tick)
	{      
